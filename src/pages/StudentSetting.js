...
const Frame4 = observer(() => {
  ...
  useEffect(() => {
    if (user) {
      setUsername(user.student.name);
      setUserEmail(user.student.username);
      setUserBirth(user.student.birth);
      setUserId(user.student.id);
    }
  ...
  const handlePasswordChange = async () => {
    if (newPassword === confirmPassword) {
      try {
        const response = await api.studentResetPassword({
          userId: userId,
    ...
    <div className="w-full h-full flex bg-purple-50">
      ...
      <div className="flex-1 p-8">
        ...
        <div className="bg-white pt-2 p-6 rounded-lg shadow-md mb-8 border border-gray-200">
          ...
          <div className="border-t border-gray-300 pt-4">
            <div className="flex items-center">
              <label className="block text-gray-700 text-sm font-bold w-24">생일</label>
              <input
                className="border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline bg-stone-200"
                type="text"
                value={userBirth}
        ...
        <div className="bg-white pt-2 p-6 rounded-lg shadow-md border border-gray-200">
          ...
          <div className="border-t border-gray-300 pt-4">
            <div className="flex items-center mb-4">
              <p className="block text-gray-700 text-sm font-bold w-24">아이디</p>
