...
const Sketch = () => {
  ...
  useEffect(() => {
    const fetchMessages = async () => {
        try {
            setScript([]);
            const response = await axios.get(`${FAST_API_BASE_URL}/chat/v1/script/${studentTaskId}`);
            const scriptData = response.data.coding;
            setScript(scriptData);

            // mainRoleName 설정
            for (const [key, item] of Object.entries(scriptData)) {
                if (key !== 'created_at' && key !== 'id') {
                    const mainRole = item[1]['인물'][0]['이름'];
                    if (mainRole) {
                        setMainRoleName(mainRole);
                        // console.log("여기", mainRole);
                        break; // 첫 번째 mainRoleName이 설정되면 종료
                    }
                }
            }
  ...
  const submitDrawing = async () => {
    ...
    tempCanvas.toBlob(async (blob) => {
      ...
      if (response.ok) {
        navigate(`/sketch-result/${studentTaskId}`, { state: { sketchUrl: url } });
      } else {
  ...
  return (
    <div className="sketch-container">
      <h1>Sketch Page</h1>
      <h3>{mainRoleName}의 몸 전체를 그려주세요!</h3>
